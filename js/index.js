...
const animateFrame = () => {
  ...
  const frameBranding = frame.querySelector('.frame__branding');
  const frameBackground = frame.querySelector('.frame__background');
  
  // Ensure the background image is always visible
  gsap.set(frameBackground, { opacity: 1 });
...
// Make init function globally accessible for the text loader
window.init = init;

// Function to prioritize only the visible images on initial load
const prioritizeVisibleImages = () => {
  // Force header background to be visible immediately - both CSS background and direct img
  const headerBackground = document.querySelector('.frame__background');
  const headerImg = headerBackground?.querySelector('img');
  ...
  if (headerImg) {
    ...
    void headerImg.offsetHeight;
  }
  
  // Get only the images that are initially visible (first section and header)
  const initialVisibleSelectors = [
    '.frame__background',
    '[data-grid-first] .grid__img'
  ];
  
  const lazyLoadSelectors = [
    '.grid:not([data-grid-first]) .grid__img'
  ];
  
  // Load priority images first - with a shorter timeout
  return preloadImages(initialVisibleSelectors.join(', '), 1000).then(() => {
    // Force a reflow to ensure background image is displayed
    if (headerBackground) {
      void headerBackground.offsetHeight;
    }
    
    // After priority images are loaded
    if (!document.querySelector('.text-loader')) {
      // If for some reason the text loader isn't present, initialize directly
      document.body.classList.remove('loading');
      init();
      window.scrollTo(0, 0);
    }
    
    // Then load the rest of the images lazily in the background
    // This won't block the UI and will happen while the user is already viewing the page
    setTimeout(() => {
      // Preload remaining images with a longer timeout since they're not immediately visible
      preloadImages(lazyLoadSelectors.join(', '), 5000);
    }, 500);
  });
};
